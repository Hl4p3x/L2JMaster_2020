...
public class AdminQuest implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_quest_reload"))
		{
			...
			if (parts.length < 2)
			{
				BuilderUtil.sendSysMessage(activeChar, "Usage: //quest_reload <questFolder>.<questSubFolders...>.questName> or //quest_reload <id>");
			}
			else
			{
		...
		else if (command.startsWith("admin_script_load"))
		{
			...
			if (parts.length < 2)
			{
				// BuilderUtil.sendSysMessage(activeChar, "Example: //script_load <questFolder>/<questSubFolders...>/<filename>.<ext> ");
				BuilderUtil.sendSysMessage(activeChar, "Example: //script_load quests/SagasSuperclass/__init__.py");
			}
			else
			{
				...
				if (file.isFile())
				{
					try
					{
						...
						BuilderUtil.sendSysMessage(activeChar, "Script Successfully Loaded.");
					}
					catch (ScriptException e)
					{
						BuilderUtil.sendSysMessage(activeChar, "Failed loading: " + parts[1]);
				...
				{
					BuilderUtil.sendSysMessage(activeChar, "File Not Found: " + parts[1]);
				}
			}
		...
		else if (command.startsWith("admin_script_unload"))
		{
			...
			if (parts.length < 2)
			{
				BuilderUtil.sendSysMessage(activeChar, "Example: //script_unload questName/questId");
			}
			else
			{
				...
				if (q != null)
				{
					if (q.unload())
					{
						BuilderUtil.sendSysMessage(activeChar, "Script Successfully Unloaded [" + q.getName() + "/" + q.getId() + "]");
					}
					else
					{
						BuilderUtil.sendSysMessage(activeChar, "Failed unloading [" + q.getName() + "/" + q.getId() + "].");
					}
				}
				else
				{
					BuilderUtil.sendSysMessage(activeChar, "The quest [" + parts[1] + "] was not found!.");
				}
			}
		}
