...
public class PcBangPoint extends AbstractNpcAI
{
	...
	private static final Map<String, int[]> TELEPORTERS = new ConcurrentHashMap<>();
	...
	// Items
	private final int PC_POINT_ITEM_INSTANT = 15358; // PC Cafe exclusive Instant Lottery 30-day limited period.
	private final int PC_POINT_ITEM_100K = 15359; // 100,000 PC Cafe points wining lottery 30-day limited period.
	private final int PC_POINT_ITEM_10K = 15360; // 10,000 PC Cafe points wining lottery 30-day limited period.
	private final int PC_POINT_ITEM_2K = 15361; // 2000 PC Cafe points wining lottery 30-day limited period.
	private final int PC_POINT_ITEM_1K = 15362; // 1000 PC Cafe points wining lottery 30-day limited period.
	
	private final int PC_POINT_ITEM_100 = 15363; // 100 PC Cafe points wining lottery 30-day limited period.
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "lottery_won_100k":
			{
				if ((player.getInventory().getItemByItemId(PC_POINT_ITEM_100K) == null))
				{
					htmltext = "lottery_fail-01.htm";
				}
				else if ((player.getPcBangPoints() + 100000) >= Config.PC_BANG_MAX_POINTS)
				{
					...
					return htmltext = "lottery_fail-02.htm";
				}
				else
				{
					takeItems(player, PC_POINT_ITEM_100K, 1);
			...
			case "lottery_won_10k":
			{
				if ((player.getInventory().getItemByItemId(PC_POINT_ITEM_10K) == null))
				{
					htmltext = "lottery_fail-01.htm";
				}
				...
				{
					takeItems(player, PC_POINT_ITEM_10K, 1);
			...
			case "lottery_won_2k":
			{
				if ((player.getInventory().getItemByItemId(PC_POINT_ITEM_2K) == null))
				{
					htmltext = "lottery_fail-01.htm";
				}
				...
				{
					takeItems(player, PC_POINT_ITEM_2K, 1);
			...
			case "lottery_won_1k":
			{
				if ((player.getInventory().getItemByItemId(PC_POINT_ITEM_1K) == null))
				{
					htmltext = "lottery_fail-01.htm";
				}
				...
				{
					takeItems(player, PC_POINT_ITEM_1K, 1);
			...
			case "lottery_won_100":
			{
				if ((player.getInventory().getItemByItemId(PC_POINT_ITEM_100) == null))
				{
					htmltext = "lottery_fail-01.htm";
				}
				...
				{
					takeItems(player, PC_POINT_ITEM_100, 1);
			...
			case "giverandom":
			{
				...
				{
					giveItems(player, PC_POINT_ITEM_INSTANT, 1);
					st.exitQuest(QuestType.DAILY, false);
				}
		...
		if (player != null)
		{
			if (POINTSSKILL.containsKey(event))
			{
				...
				else if (player.getPcBangPoints() >= POINTSSKILL.get(event)[2])
				{
					...
					player.setPcBangPoints(cost);
					SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.USING_S1_PCPOINT);
					smsgpc.addLong(POINTSSKILL.get(event)[2]);
					player.sendPacket(smsgpc);
			...
			else if (PETSKILL.containsKey(event))
			{
				...
				else if (player.getPcBangPoints() >= PETSKILL.get(event)[2])
				{
					...
					player.setPcBangPoints(cost);
					SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.USING_S1_PCPOINT);
					smsgpc.addLong(PETSKILL.get(event)[2]);
					player.sendPacket(smsgpc);
			...
			else if (TELEPORTERS.containsKey(event))
			{
				if (player.getPcBangPoints() >= TELEPORTERS.get(event)[3])
				{
					...
					player.setPcBangPoints(cost);
					SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.USING_S1_PCPOINT);
					smsgpc.addLong(TELEPORTERS.get(event)[3]);
					player.sendPacket(smsgpc);
